...
if (isset($_POST['add'])) {
    ...
    if (!move_uploaded_file($_FILES["file"]["tmp_name"], './img/service/' . $_FILES["file"]["name"])) {
    ...
    $sql = ("INSERT INTO service (Name, Description, Photo) VALUES (?, ?, ?)");
...
if (isset($_POST['edit'])) {
    ...
    if (strcasecmp($_FILES["file"]["tmp_name"], '') != 0) {
        ...
        $sql = ("UPDATE `service` SET Name=?,Description=?,Photo=? WHERE id=?");
        $query = $pdo->prepare($sql);
